...
const findGroup = (meetingId, { name, model }) => {
  if (hasMeeting(meetingId)) {
    ...
    return groupIds.find(groupId => {
      ...
      return group.name === name && group.model === model;
    });
  }
...
const addGroup = (meetingId, {
  ...
  return new Promise((resolve, reject) => {
    if (hasMeeting(meetingId)) {
      if (findGroup(meetingId, { name, model })) {
        logger.warn('database', 'duplicated', 'group', { meetingId, name, model });
      ...
      api.call('createGroup').then(response => {
        ...
        logger.debug('database', 'added', 'group', { meetingId, groupId, name, model });
        ...
        sender.send('groupAdded', meetingId, { groupId, name, model });

        resolve(database[meetingId].groups[groupId]);
      }).catch(() => {
        logger.error('database', 'adding', 'group', { meetingId, name, model });
  ...
  return new Promise((resolve, reject) => {
    if (hasGroup(meetingId, groupId)) {
      ...
      api.call('createGroupPad', { groupID: groupId, padName: name }).then(response => {
        ...
        sender.send('padAdded', meetingId, { groupId, padId, name });
