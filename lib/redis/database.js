...
const hasCapacity = (meetingId, groupId) => {
  if (hasGroup(meetingId, groupId)) {
    ...
    return sessions.length < capacity;
...
const onPadUpdate = (meetingId, groupId, padId, rev) => {
  if (hasGroup(meetingId, groupId)) {
    api.call('getHTML', { padID: padId, rev }).then(response => {
      logger.info(ids.PAD, 'content', { meetingId, groupId, padId, rev });

      const { html: content } = response;
      sender.send('padContent', meetingId, { groupId, padId, rev, content });
    ...
    if (hasGroup(meetingId, groupId)) {
      ...
      api.call('createGroupPad', {
        ...
        database[meetingId].groups[groupId].pads[padId] = {
          text: '',
          diff: (meetingId, groupId, padId, text) => {
...
const updatePad = (padId, { authorId, rev, changeset, text }) => {
  return new Promise((resolve, reject) => {
    ...
    if (user && pad) {
      ...
      database[meetingId].groups[groupId].pads[padId].diff(meetingId, groupId, padId, text);
