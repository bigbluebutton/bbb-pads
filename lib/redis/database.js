...
const createMeeting = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    logger.debug('database', 'created', 'meeting', { meetingId });
  ...
  return new Promise((resolve, reject) => {
    if (hasMeeting(meetingId)) {
      ...
      const update = () => {
        ...
        logger.debug('database', 'deleted', 'meeting', { meetingId });
      ...
      if (promises.length !== 0) {
        Promise.all(promises).then(() => update()).catch(() => {
          logger.error('database', 'deleting', 'meeting', { meetingId });
  ...
  return new Promise((resolve, reject) => {
    if (hasMeeting(meetingId)) {
      ...
      api.call('createAuthor', { name }).then(response => {
        ...
        logger.debug('database', 'created', 'user', { meetingId, userId, authorId });
        ...
        logger.error('database', 'creating', 'user', { meetingId, userId });
  ...
  return new Promise((resolve, reject) => {
    if (hasUser(meetingId, userId)) {
      ...
      const update = () => {
        ...
        logger.debug('database', 'deleted', 'user', { meetingId, userId, authorId });
      ...
      if (promises.length !== 0) {
        Promise.all(promises).then(() => update()).catch(() => {
          logger.error('database', 'deleting', 'user', { meetingId, userId });
  ...
  return new Promise((resolve, reject) => {
    if (hasMeeting(meetingId)) {
      if (findGroup(meetingId, { externalId, model })) {
        logger.warn('database', 'duplicated', 'group', { meetingId, externalId, model });
      ...
      api.call('createGroup').then(response => {
        ...
        logger.debug('database', 'created', 'group', { meetingId, groupId, externalId, model });
        ...
        logger.error('database', 'creating', 'group', { meetingId, externalId, model });
  ...
  return new Promise((resolve, reject) => {
    if (hasGroup(meetingId, groupId)) {
      ...
      const update = () => {
        ...
        logger.debug('database', 'deleted', 'group', { meetingId, groupId });
      ...
      if (promises.length !== 0) {
        Promise.all(promises).then(() => update()).catch(() => {
          logger.error('database', 'deleting', 'group', { meetingId, groupId });
  ...
  return new Promise((resolve, reject) => {
    if (hasGroup(meetingId, groupId)) {
      ...
      api.call('createGroupPad', { groupID: groupId, padName: name }).then(response => {
        logger.debug('database', 'created', 'pad', { meetingId, groupId, padId });
        ...
        logger.error('database', 'creating', 'pad', { meetingId, padId });
...
const deletePad = (meetingId, groupId, padId) => {
  if (hasPad(meetingId, groupId, padId)) {
    ...
    logger.debug('database', 'deleted', 'pad', { meetingId, groupId, padId });
  ...
  return new Promise((resolve, reject) => {
    if (hasPermission(meetingId, groupId, userId)) {
      ...
      api.call('createSession', groupId, authorId, 0).then(response => {
        ...
        logger.debug('database', 'created', 'session', { meetingId, groupId, userId, sessionId });
        ...
        logger.error('database', 'creating', 'session', { meetingId, groupId, userId });
  ...
  return new Promise((resolve, reject) => {
    if (hasSession(meetingId, groupId, userId)) {
      ...
      api.call('deleteSession', { sessionID: sessionId }).then(response => {
        logger.debug('database', 'deleted', 'session', { meetingId, groupId, userId });
        ...
        logger.error('database', 'deleting', 'session', { meetingId, groupId, userId });
