...
const handleMeetingEnded = (header, body) => {
  ...
  database.deleteMeeting(meetingId).then(response => {
...
const handleUserLeft = (header, body) => {
  ...
  database.deleteUser(meetingId, { userId }).then(response => {
...
const handleGroupCreate = (header, body) => {
  ...
  database.createGroup(meetingId, { externalId, model }).then(response => {
    logger.info('group', 'created', meetingId, externalId, model);
...
const handlePadCreate = (header, body) => {
  ...
  database.createPad(meetingId, groupId, { name }).then(response => {
...
const handleSessionCreate = (header, body) => {
  ...
  database.createSession(meetingId, groupId, userId).then(response => {
...
const handlePadSettingsLoaded = (header, body) => {
  logger.debug('pad', 'settings', 'loaded', body);
};

const handlePadShutdown = (header, body) => {
  logger.debug('pad', 'shutdown', body);
};

const handlePadConfigured = (header, body) => {
  logger.debug('pad', 'configured', body);
};

const handlePadServerCreated = (header, body) => {
  logger.debug('pad', 'server', 'created', body);
};

const handlePadServerClosed = (header, body) => {
  logger.debug('pad', 'server', 'closed', body);
};

const handlePadAccessChecked = (header, body) => {
  logger.debug('pad', 'access', 'checked', body);
};

const handlePadCreated = (header, body) => {
  logger.debug('pad', 'created', body);
};

const handlePadLoaded = (header, body) => {
  logger.debug('pad', 'loaded', body);
};
...
const handlePadCopied = (header, body) => {
  logger.debug('pad', 'copied', body);
};

const handlePadRemoved = (header, body) => {
  logger.debug('pad', 'removed', body);
};

const handlePadUserLeft = (header, body) => {
  logger.debug('pad', 'user', 'left', body);
};
