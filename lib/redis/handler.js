...
const handlePadSettingsLoaded = (header, body) => {
  logger.info('pad', 'settings', 'loaded', JSON.stringify(body));
};

const handlePadShutdown = (header, body) => {
  logger.info('pad', 'shutdown', JSON.stringify(body));
};

const handlePadConfigured = (header, body) => {
  logger.info('pad', 'configured', JSON.stringify(body));
};

const handlePadServerCreated = (header, body) => {
  logger.info('pad', 'server', 'created', JSON.stringify(body));
};

const handlePadServerClosed = (header, body) => {
  logger.info('pad', 'server', 'closed', JSON.stringify(body));
};

const handlePadAccessChecked = (header, body) => {
  logger.info('pad', 'access', 'checked', JSON.stringify(body));
};
...
const handlePadLoaded = (header, body) => {
  logger.info('pad', 'loaded', JSON.stringify(body));
};
...
const handlePadUpdated = (header, body) => {
  ...
  database.updatePad(padId, { authorId, rev, changeset }).then(response => {
    logger.info('pad', 'updated', padId, JSON.stringify(response));
  }).catch(() => logger.error('pad', 'updating', JSON.stringify(body)));
};

const handlePadCopied = (header, body) => {
  logger.info('pad', 'copied', JSON.stringify(body));
};

const handlePadRemoved = (header, body) => {
  logger.info('pad', 'removed', JSON.stringify(body));
};

const handlePadUserLeft = (header, body) => {
  logger.info('pad', 'user', 'left', JSON.stringify(body));
};
